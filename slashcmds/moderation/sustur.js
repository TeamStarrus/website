...
module.exports = {
  ...
  run: async (client, interaction) => {
    if (!interaction.member.permissions.has(`MODERATE_MEMBERS`)) return interaction.followUp({content: `${client.config.s.ticks.red} Bu komutu kullanabilmek için \`Üyelere Zamanaşımı Uygula\` iznine sahip olmalısın!`, ephemeral: true});
    ...
    if (!response) return interaction.followUp({content: `${client.config.s.ticks.red} Bu üyeyi sunucuda susturamıyorum.`});
    await interaction.followUp({content: `${client.config.s.ticks.green} ${member}, ${ms(time, {long: true})} süreyle susturuldu!`});
  },
};
