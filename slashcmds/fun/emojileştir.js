...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption(option => option.setName(`yazı`).setDescription(`Emojileştirmek istediğin yazıyı yaz.`).setRequired(true)),
  run: async (client, interaction) => {
    ...
    await interaction.followUp({content: text.split(``).map(c => mapping[c] || c).join(``)});
  },
};
